...
class RAGPipelineService:
    def __init__(
        ...
        # Sub-services
        self.chunking_service = ChunkingService(chunk_size=chunk_size, chunk_overlap=chunk_overlap)

        self.embedding_service = EmbeddingService(openai_api_key=embedding_api_key, model=embedding_model)
        ...
        self.search_service = SemanticSearchService(vector_store_service=self.vector_store_service)

        self.ranking_service = RankingService(score_threshold=0.0)

        # Initialize LLMService with memory support; pass session_id_getter for per-session memory
        self.llm_service = LLMService(
            ...
            retriever=self.search_service.semantic_search  # optional: LLMService can call this for context
        )

        # Agent (langgraph) will be created after vector store loaded
        self.agent_executor = None

        # Initialize (load or build vector store then create agent)
        self._initialize()

    def _initialize(self) -> None:
        """Load or build vector store, then create agent."""
        try:
            if self.vector_store_service.should_rebuild():
                logger.info("Rebuilding vector store (files changed or missing)...")
                self._build_vectorstore()
            else:
                logger.info("Loading existing vector store")
                self.vector_store_service.load_vectorstore()
        except Exception:
            logger.exception("Vector store load/build failed; continuing but agent may not work correctly")

        # create agent with a retriever tool that uses both KB and memory
        self._create_agent()
    ...
    def _build_vectorstore(self) -> None:
        ...
        if not chunks:
            logger.warning("No chunks produced during chunking")
